...
import { ArrowLeft, ArrowRight, Calendar, CreditCard, User, Hash, ReceiptText } from 'lucide-react';

export const InstallmentData = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const { amount, phone, selectedPlan } = location.state || {};
    ...
    if (!preview) return <p className="text-center py-10">Loading...</p>;
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-slate-700">{preview.merchant.phone}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm text-slate-400">
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-slate-700">{preview.package_name}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-indigo-50/50 rounded-4xl p-6 border border-indigo-100 mb-8">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between text-slate-600 italic">
                                    ...
                                    <span>${parseFloat(preview.amount).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-slate-600 italic">
                        ...
                        <div className="mb-8">
                            ...
                            <div className="space-y-3">
                                {preview.installments.map((inst, idx) => (
                                    <div key={inst.installment_no} className="flex items-center gap-4 group">
                                        ...
                                        <div className="flex-1 bg-white border border-slate-100 p-4 rounded-2xl flex justify-between items-center group-hover:border-indigo-200 transition">
                                            <div>
                                                <p className="font-bold text-slate-800 text-sm">Installment #{inst.installment_no}</p>
                                                <p className="text-xs text-slate-400 flex items-center gap-1">
                                                    <Calendar size={12} /> Due: {inst.due_date}
                                                </p>
                                            </div>
