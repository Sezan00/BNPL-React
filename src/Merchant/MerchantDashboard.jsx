...
export const MerchantDashboard = () => {
  ...
  useEffect(() => {
    const fetchMerchantData = async () => {
      try {
        const token = localStorage.getItem('token');
        const res = await axios.get('https://bnpl-backend.sezan.xyz/api/merchant/profile', {
          headers: {
  ...
  useEffect(() => {
    const fetchTransactionData = async () => {
      const token = localStorage.getItem('token');
      const res = await axios.get(`https://bnpl-backend.sezan.xyz/api/merchant/transaction`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
  ...
  const handleLogout = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(`https://bnpl-backend.sezan.xyz/api/merchant/logout`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
